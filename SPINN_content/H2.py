...
def render_page():
    try:
        from SPINN_model.H2.VGM_PTF import VGM_PTF
        from SPINN_model.H2.M3_PTF  import M3_PTF
        from SPINN_model.H2.B_FXW_PTF import B_FXW_PTF
    ...
    #)
    st.subheader('H2-PTFs: Need to know the soil texture information (sand, silt, clay [×100%])')
    ...
    if submitted:
       if sand+silt+clay<=1.01 and sand+silt+clay>=0.99:
          ...
          [VGM_Para,nan]=VGM_PTF([[sand,silt,clay]])
          ...
          [M3_Para,nan]=M3_PTF([[sand,silt,clay]])
          ...
          [B_FXW_Para,nan]=B_FXW_PTF([[sand,silt,clay]])
          ...
          st.write('input error (sand+silt+clay should equal to 1)')
