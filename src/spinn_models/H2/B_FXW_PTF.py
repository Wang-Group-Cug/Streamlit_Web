...
def B_FXW_PTF(Texture):
    ...
    class FNN_test(nn.Module):
        ...
        def forward(self,x):
            ...
            return out2
    FNN=FNN_test(input_size=3,hidden_size=226,output_size=5,layers=3)
    FNN.to(device=device)

    model_path_swrc = Path(__file__).parent / "B_FXW_H2_SWRC.pth"
    ...
    class FNN_test(nn.Module):
        ...
        def forward(self,x):
            ...
            return out2
    FNN=FNN_test(input_size=3,hidden_size=226,output_size=2,layers=3)
    FNN.to(device=device)

    model_path_k = Path(__file__).parent / "B_FXW_H2_K.pth"
