...
def VGM_PTF(Texture):
    ...
    class FNN_test(nn.Module):
        ...
        def forward(self,x):
            ...
            return out2
    FNN=FNN_test(input_size=3,hidden_size=156,output_size=4,layers=2)
    FNN.to(device=device)

    model_path_swrc = Path(__file__).parent / "VGM_H2_SWRC.pth"
    ...
    class FNN_test(nn.Module):
        ...
        def forward(self,x):
            ...
            out=self.seq(x)
            out2=torch.zeros(out.shape)
            out2[:, [0]] = -4 + 8.8 * out[:, [0]]      # Ks
            
            return out2
    FNN=FNN_test(input_size=3,hidden_size=156,output_size=1,layers=2)
    FNN.to(device=device)

    model_path_k = Path(__file__).parent / "VGM_H2_K.pth"
